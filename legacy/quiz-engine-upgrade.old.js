...
async function loadData() {
  ...
  try {
    ...
    // ── Lưu vào 2 nơi: toàn cục + State ──────────────────────
    window.quizRepository = data;
    State.quizData = data;

    console.info(
      `%c[EduQuiz] ✅ quiz_data.json nạp thành công!`,
      'color:#00c9a0;font-weight:700'
    );
    _logRepositorySummary(data);

  } catch (err) {
    console.warn('[EduQuiz] ⚠ Không tải được quiz_data.json. Dùng DEMO_DATA.', err.message);
...
function _logRepositorySummary(data) {
  ...
  data.categories?.forEach(cat => {
    cat.levels?.forEach(lv => {
      Object.values(lv.minitests || {}).forEach(qs => { totalQ += qs.length; });
    });
  });
  ...
  const refreshMeta = () => {
    ...
    if (qs.length) {
      ...
      const typeChips = Object.entries(typeCounts)
        .map(([t, n]) => `<span class="chip purple">${typeLabels[t] || t}: ${n}</span>`)
        .join('');

      el.innerHTML = `
        <span class="chip green">📝 ${qs.length} câu</span>
        ${typeChips}
      `;
    } else {
    ...
    const btn    = document.getElementById('btnStart');
    if (btn) {
      btn.disabled = !(qs.length && name && cls && school);
...
function startExam() {
  ...
  // ── Lấy mảng câu hỏi chính xác từ quizRepository ─────────
  const cat = _findCategory(catId);
  const lv  = cat?.levels?.find(l => l.id === lvlId);
  const rawQs = lv?.minitests?.[mtName];

  if (!rawQs || rawQs.length === 0) {
    alert('⚠ Không tìm thấy câu hỏi cho bài này. Vui lòng kiểm tra quiz_data.json.');
    return;
  }
